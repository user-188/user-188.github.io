...
export const Projects = () => {
    ...
    const projects: Project[] = [
        ...
        },
        {
            ...
            image: "/database_project_banner.png",
        }
    ];
        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {projects.map((project, index) => (
                    <div
                        key={index}
                        className={`group rounded-3xl overflow-hidden bg-surface border border-text/10 hover:border-text/20 transition-all duration-300 ${index === 0 ? 'lg:col-span-2' : ''}`}
                    >
                        ...
                        <div className={`relative overflow-hidden bg-black/20 ${index === 0 ? 'min-h-96' : 'min-h-64'}`}>
                            ...
                            {project.demo && (
                                <button
                                    ...
                                    className="absolute top-4 right-4 z-30 flex items-center gap-2 px-4 py-2 rounded-full bg-black/50 backdrop-blur-md border border-text/20 text-white hover:bg-white/10 transition-colors"
                                >
                        ...
                        <div className="p-8 relative z-20">
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    ...
                                    <h3 className="text-2xl md:text-3xl font-bold text-text group-hover:text-primary transition-colors">{project.title}</h3>
                                </div>
                                {project.link && (
                                    <a href={project.link} target="_blank" rel="noreferrer" className="p-2 bg-text/5 rounded-full hover:bg-text/20 transition-colors text-text inline-block">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {project.technologies.map((tech, i) => (
                                    <span key={i} className="px-3 py-1 text-xs md:text-sm rounded-lg bg-surfaceHighlight border border-text/10 text-muted">
                                        {tech}
                                    </span>
                                ))}
                            </div>
